...
2. Separating the user interface from program logic
    ...
    * Changes made inside the class WordSet don't affect the class UserInterface (as long as names of methods that class user interface uses). This is because the user interface uses WordSet through the methods that it provides — these are called its public interfaces
